    ...
    <table class="min-w-full text-sm text-left border-collapse">
      ...
      <tbody>
        ${schedules.map(schedule => {
          ...
          let start = schedule.start_time ? new Date(schedule.start_time) : null;
          let end = schedule.end_time ? new Date(schedule.end_time) : null;

          // Apply -8 hours for Room 315
          if (schedule.room === "315") {
            if (start) start.setHours(start.getHours() - 8);
            if (end) end.setHours(end.getHours() - 8);
          }

          // Ensure earlier time comes first
          let first = (start && end && start > end) ? end : start;
          let second = (start && end && start > end) ? start : end;

          const startTime = first ? first.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : '-';
          const endTime = second ? second.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : '-';
