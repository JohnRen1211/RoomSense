...
const String SUPABASE_TABLE_NAME = "fingerprint_logs"; // Name of the table you created
...
// User data (replace with your actual users)
const int MAX_USERS = 5;
String names[MAX_USERS + 1] = { "", "Alice", "Bob", "Charlie", "Diana", "Eve" };
  ...
  if (finger.getImage() == FINGERPRINT_OK) {
    if (finger.image2Tz() == FINGERPRINT_OK && finger.fingerFastSearch() == FINGERPRINT_OK) {
      ...
      if (id >= 1 && id <= MAX_USERS) {
        ...
        String status = isTimeIn[id] ? "Time Out" : "Time In";
        isTimeIn[id] = !isTimeIn[id]; // Toggle status

        Serial.printf("👆 Finger detected!\n✅ Match Found: ID: %d - %s\n🕒 %s recorded.\n", id, name.c_str(), status.c_str());
...
void sendToSupabase(int finger_id, String name, String status) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String idAndName = "ID" + String(finger_id) + "-" + name;

    // Construct the JSON payload
    String jsonPayload = "{\"finger_id_name\": \"" + idAndName + "\", \"status\": \"" + status + "\"}";
